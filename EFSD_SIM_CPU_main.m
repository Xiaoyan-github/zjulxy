...
sample = imresize(Rawimage,scaleFactor,'Method','bilinear');
...
sigma = 1; % positive number, e.g. 1
attFun = 1-amp*exp(-cycl.^2/(2*sigma^2));
[~, OTFde] = generatePSFOTF(xsize, ysize, pixelSize, NA, lambdaEmi);
...
% % % % deconvolution operation (optional)
[psf, ~] = generatePSFOTF(xsize, ysize, pixelSize, 2*NA, lambdaEmi);
EFSDSIMDec = deconvlucy(EFSDSIM,psf,3);
imwrite(EFSDSIMDec,strcat(filepath,'Deconvoluted EFSD-SIM.tif'));